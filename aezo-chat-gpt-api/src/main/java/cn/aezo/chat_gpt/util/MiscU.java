...
public final class MiscU {
	...
	public static <K, V> Map<K, V> removeNullAndBlankStrValue(Map<K, V> map) {
		if (ValidU.isEmpty(map)) {
			return map;
		}
		...
		while (iter.hasNext()) {
			entry = iter.next();
			if (null == entry.getValue() || entry.getValue() instanceof String && "".equals(entry.getValue())) {
				iter.remove();
			}
		}
	...
	public static class Instance {
		...
		public static <K, V> Map<String, V> toMapExt(boolean yesLink, Object... data) {
			if (data.length == 1 && data[0] instanceof Map) {
				// return UtilGenerics.<String, V>checkMap(data[0]); // TODO 校验
			}
			if (data.length % 2 == 1) {
			...
			Map<String, V> map = yesLink ? new LinkedHashMap<>() : new HashMap<String, V>();
			for (int i = 0; i < data.length;) {
				map.put((String) data[i++], (V) data[i++]);
			}
			return map;
