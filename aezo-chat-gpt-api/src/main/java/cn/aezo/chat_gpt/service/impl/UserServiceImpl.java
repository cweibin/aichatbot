...
public class UserServiceImpl implements UserService {
    ...
    @Value("${aezo-app-common.enc-pass-key:${sq-app-common.enc-pass-key:}}")
    private String encPassKey;

    @Value("${aezo-chat-gpt.chat.invite-num:${sq-mini-tools.chat.invite-num:10}}")
    private String chatinviteNum;

    @Value("${aezo-chat-gpt.chat.invite-max:${sq-mini-tools.chat.invite-max:5}}")
    ...
    public String checkAndGetUserId(String username, String password) {
        ...
        List<Map<String, Object>> list = getUserByUsername(username);
    ...
    public List<Map<String, Object>> getUserByUsername(String username) {
        return jdbcTemplate.queryForList(
                "select id, password from mt_user_info " +
                        " where valid_status = 1 and (username = ? or email = ? or mobile = ?)", username, username, username);
    }
    ...
    @Override
    public Result loginByWxMa(Map<String, Object> params) {
        ...
        try {
            ...
            return this.getUserIdByOpenid("WxMa", openid, inviterUserId);
    ...
    public Result loginByWxMp(Map<String, Object> params) {
        ...
        final WxMpService wxService = SqThirdAppConfig.getMpService(appid);
        try {
            WxOAuth2AccessToken wxOAuth2AccessToken = wxService.getOAuth2Service().getAccessToken(code);
            String openid = wxOAuth2AccessToken.getOpenId();
            return this.getUserIdByOpenid("WxMp", openid, inviterUserId);
        } catch (WxErrorException e) {
            log.error("登录出错", e);
            return Result.failure("登录出错");
        }
    ...
    public Map<String, Object> register(String username, String password, String inviterUserId) {
        ...
        } else if(Convert.toLong(username, 0L).compareTo(0L) != 0) {
            mobile = username;
            username = this.getRandomUsername();
        }
        if(ValidU.isNotEmpty(password)) {
            password = encPassword(password);
        }
        // 默认注册为一级普通会员
        jdbcTemplate.update("insert into mt_user_info(id, username, password, email, mobile, " +
                " nick_name, user_level, inviter, version, create_time, update_time) " +
                " values(?, ?, ?, ?, ?, ?, ?, ?, 0, now(), now())",
                userId, username, password, email, mobile, username, "M1", inviterUserId);
        boolean inviterReward = this.inviterReward(inviterUserId, userId);
        if(!inviterReward) {
            log.error("处理邀请奖励失败. {} {}", inviterUserId, userId);
        }
        return MiscU.Instance.toMap("userId", userId);
    }
    
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> registerByOpenid(String openType, String openid, String inviterUserId) {
        String username = this.getRandomUsername();
        Map<String, Object> registerMap = this.register(username, null, inviterUserId);
        String userId = (String) registerMap.get("userId");
        String id = IdUtil.getSnowflakeNextIdStr();
        jdbcTemplate.update("insert into mt_user_oauth(id, user_id, login_type, openid, create_time, update_time) " +
                "values(?, ?, ?, ?, now(), now())", id, userId, openType, openid);
        return MiscU.Instance.toMap("id", id, "userId", userId,
                "openType", openType, "openid", openid);
    }
    ...
    @Override
    public Result bind(String username, String password) {
        String loginIdAsString = StpUtil.getLoginIdAsString();
        if(ValidU.isNotEmpty(password)) {
            password = encPassword(password);
        }
        String setSql = " ,password = ?";
        if(username.contains("@")) {
            setSql += " ,email = ? ";
        } else {
            setSql += " ,mobile = ? ";
        }
        int count = jdbcTemplate.update("update mt_user_info set update_time=now()" + setSql + " where id = ?", password, username, loginIdAsString);
        return count > 0 ? Result.success() : Result.failure();
    }

    private Result getUserIdByOpenid(String openType, String openid, String inviterUserId) {
        // 查询是否存在相应类型的 openid
        List<Map<String, Object>> userOauthList = jdbcTemplate.queryForList(
                "select id \"id\", user_id \"userId\", openid \"openid\" from mt_user_oauth ua where " +
                        " ua.valid_status = 1 and ua.login_type = ? and ua.openid = ?", openType, openid);
        if(ValidU.isEmpty(userOauthList)) {
            UserService userService = SpringU.getBean(UserService.class);
            Map<String, Object> userOauth = userService.registerByOpenid(openType, openid, inviterUserId);
            if(userOauth != null) {
    ...
    private boolean inviterReward(String inviterUserId, String userId) {
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                AssetChatMapper chatMapper = SpringU.getBean(AssetChatMapper.class);
    ...
    private String getRandomUsername() {
        ...
        do {
            ...
            if(Convert.toLong(username, 0L).compareTo(0L) != 0 || "0000000000".equals(username)) {
                count = 1;
                continue;
            }
            username = "mt_" + username;
    ...
    public static void main(String[] args) {
        ...
        // ad35cc0c10b2520d4577602699f1ca94598c8e95
